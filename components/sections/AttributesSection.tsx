...
export const AttributesSection = ({attributes, stigmas}: AttributesSectionProps) => {
  const coreAttributes = attributes.filter(a => a.type === "core");
  const advancedAttributes = attributes.filter(a => a.type === "secondary");
...
const AttributeRow = ({attribute, delay}: AttributeRowProps) => {
  ...
  const Icon = icons[attribute.category];
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="font-mono text-xs text-primary">{attribute.level}/10</span>
      </div>
      <div className="attribute-bar">
        <motion.div
          className="attribute-fill"
          initial={{width: 0}}
          whileInView={{width: `${attribute.level * 10}%`}}
          viewport={{once: true}}
