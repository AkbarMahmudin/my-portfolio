...
export const HeroSection = ({profile, ctas}: HeroSectionProps) => {
            ...
            <div className="p-8 md:p-12 text-left">
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-4 mt-6">
                  <div
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-secondary/30 border border-border/50 backdrop-blur-sm">
                    <span className="font-mono text-xs text-muted-foreground">ROLE:</span>
                    <span className="font-mono text-sm text-accent">{profile.role}</span>
                  </div>
                  <div
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-secondary/30 border border-border/50 backdrop-blur-sm">
                ...
                <p className="text-muted-foreground mt-6 max-w-lg font-mono text-sm leading-relaxed">
                  <span className="text-primary/60">//</span> {profile.headline}
                </p>
              </motion.div>
              ...
              >
                ...
                {ctas?.map((cta, index) => (
                  <Link href={cta?.url} className={cta?.isPrimary ? "btn-system-primary" : "btn-system"}
                        target={cta?.isBlank ? "_blank" : "_self"} key={index} suppressHydrationWarning>
